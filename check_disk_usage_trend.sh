...
for DISK in "${DISKS[@]}"; do
    ...
    if [ ! -f $LOG_FILE ]; then
        echo "Log file not found for $DISK!"
        continue
    fi
    ...
    if [ $PREDICTED_USAGE -gt $THRESHOLD ]; then
        echo "Disk usage on $DISK is predicted to exceed ${THRESHOLD}% within ${DAYS_AHEAD} days. Current usage: ${CUR_USAGE}%. Predicted usage: ${PREDICTED_USAGE}%." | mail -s "Disk Usage Alert for $DISK" $EMAIL
    fi
done
